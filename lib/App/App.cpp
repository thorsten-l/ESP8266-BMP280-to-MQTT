...
void App::defaultConfig()
{
  ...
  appcfg.i2c_sensor_address = DEFAULT_I2C_SENSOR_ADDRESS;
...
void App::loadConfig()
{
  ...
  sprintf(buffer, appcfg.mqtt_intopic, ESP.getChipId());
  strcpy(appcfg.mqtt_intopic, buffer);
  sprintf(buffer, appcfg.mqtt_outtopic, ESP.getChipId());
  strcpy(appcfg.mqtt_outtopic, buffer);
  sprintf(buffer, appcfg.mqtt_topic_json, ESP.getChipId());
  strcpy(appcfg.mqtt_topic_json, buffer);
  //------------------------------------------------------------
}
...
void App::writeConfig()
{
  ...
  else
  {
    ...
    j.writeEntry(A_i2c_sensor_address, appcfgWR.i2c_sensor_address);
...
bool App::loadJsonConfig(const char *filename)
{
  ...
  if (j.readHeader())
  {
    ...
    while (readError == false && j.readAttributeName(attributeName) == true)
    {
      ...
      readError |= j.readEntryInteger(attributeName, A_i2c_sensor_address, &appcfgRD.i2c_sensor_address);
    }
  }
